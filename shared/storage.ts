...
// --- 迁移和格式转换（统一从 utils/migration 导入并导出） ---
import { migratePomodoro as _migratePomodoro, migrateTaskDefs as _migrateTaskDefs, logsToFirestore as _logsToFirestore, logsFromFirestore as _logsFromFirestore } from './utils/migration';
export const migratePomodoro = _migratePomodoro;
export const migrateTaskDefs = _migrateTaskDefs;
export const logsToFirestore = _logsToFirestore;
export const logsFromFirestore = _logsFromFirestore;
...
export async function resetAllData(storage: StorageInterface, cloud?: CloudSyncService): Promise<void> {
  ...
  const taskDefs = migrateTaskDefs(existing.taskDefs) || DEFAULT_TASK_DEFS;
