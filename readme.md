...
- **跨设备同步** — Google / 邮箱密码登录，双向自动同步，日志去重合并，番茄钟原子同步
- **统计分析** — maxEnergy / 消耗量 / 番茄数趋势图 + 操作日志
- **76 个自动化测试** — 核心逻辑 + 存储 + 时间工具 + 类型 + Puppeteer UI 全覆盖
...
├── logic.ts                   #   核心纯函数 (衰减/恢复/番茄检查/完美一天/精力上限)
├── utils/time.ts              #   时间工具
├── components/                #   MainDashboard, StatsPage, RulesPage, SettingsPage, MenuPanel
└── public/                    #   共享静态资源

tests/                         # 测试 (76 case)
├── logic.test.ts              #   核心逻辑测试
├── storage.test.ts            #   存储抽象测试
├── time.test.ts               #   时间工具测试
├── types.test.ts              #   类型测试
└── ui.test.ts                 #   Puppeteer UI 测试
...
| 登录 | chrome.identity OAuth + Email/Password | signInWithPopup + Email/Password |
| 提醒 | 新开标签页 | 当前页全屏覆盖 |
